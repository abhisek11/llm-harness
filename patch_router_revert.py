...
router_chat_search = """async *chat(messages: Message[], options: ChatOptions & { model?: string } = {}): AsyncGenerator<string> {
...
sorted_search = """const sorted = targetProvider && targetProvider !== 'auto'
      ? [
...
sorted_replace = """const sorted = options.model && options.model !== 'auto'
      ? [
